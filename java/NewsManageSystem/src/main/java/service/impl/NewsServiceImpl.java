	...
	public List<List<News>> findLatestNewsByTid(Map<Integer, Integer> topicsMap) throws SQLException {
		...
		try {
			...
			if (topicsMap != null && topicsMap.size() != 0) {
				...
				Iterator<Entry<Integer, Integer>> topics = topicsMap.entrySet().iterator();
				while (topics.hasNext()) {
					Entry<Integer, Integer> oneTopic = topics.next();
					result.add(newsDao.getLatestNewsByTID(oneTopic.getKey(), oneTopic.getValue()));
				}
			}
			return result;
		} catch (SQLException e) {
	...
	public void findPageNews(Page pageObj) throws SQLException {
		...
		try {
			...
			if (totalCount > 0) {
				...
				List<News> newsList = newsDao.getPageNewsList(pageObj.getCurrPageNo(), pageObj.getPageSize());
