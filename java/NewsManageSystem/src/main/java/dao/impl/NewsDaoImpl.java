...
public class NewsDaoImpl extends BaseDao implements NewsDao {
	...
	public List<News> getAllnewsByTname(String tname) throws SQLException {
		...
		String sql = "SELECT `nid`, `ntid`, `ntitle`, `nauthor`,"
				+ " `ncreateDate`, `nsummary`, `tname` FROM `NEWS`, `TOPIC`"
				+ " WHERE `NEWS`.`ntid` = `TOPIC`.`tid` AND `TOPIC`.`tname` = ?" + " ORDER BY `ncreateDate` DESC";
		try {
	...
	public int addNews(News news) throws SQLException {
		...
		try {
			result = executeUpdate(sql,
					new Object[] { news.getNtid(), news.getNtitle(), news.getNauthor(), news.getNcontent(),
							news.getNsummary(), news.getNcreatedate(), news.getNmodifydate(), news.getNpicpath() });
		} catch (SQLException e) {
			e.printStackTrace();
			throw e;
		}
	...
	public int updateNews(News news) throws SQLException {
		...
		int result = 0;
		try {
			result = executeUpdate(sql, new Object[] { news.getNtid(), news.getNtitle(), news.getNauthor(),
					news.getNcontent(), news.getNsummary(), news.getNmodifydate(), news.getNpicpath(), news.getNid() });
		} catch (SQLException e) {
			e.printStackTrace();
			throw e;
		}
